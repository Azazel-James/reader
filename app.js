...
// 15 Update the displayed table with the verification status for each line
function updateTableDisplay(id, ok) {
  // Finds a row by it's data-id then gets the signature status cell
  const row = document.querySelector(`tr[data-id="${id}"]`);
  const statusCell = row.querySelector(".signature-status");

  // Updates the cell content with a check or cross mark depending on the verification results
  if (ok) {
    statusCell.innerHTML = "&#9989;"; // Checkmark
  } else {
    statusCell.innerHTML = "&#10060;"; // Cross mark
  }
}
